...
folder are mapped to the application's root namespace, 'GirocheckMobile'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "GirocheckMobile"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'GirocheckMobile' in the same way
as `"GirocheckMobile/sass/src"`.
